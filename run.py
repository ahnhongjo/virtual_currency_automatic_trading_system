...
# 오늘 시가 함수
def get_today_open(df):
    open_to = df['open']
    return open_to[-1]


# 1일전 시가 함수
def get_yesterday_open(df):
    open_yes = df['open']
    return open_yes[-2]


# 1일전 종가 함수
def get_yesterday_close(df):
    close = df['close']
    return close[-2]


# 30일 평균 거래량 함수
def get_aver_volume(df):
    volume_aver_30 = df['volume'].rolling(window=30).mean()
    return volume_aver_30[-2]


# 오늘 거래량 함수
def get_to_volume(df):
    volume = df['volume']
    return volume[-1]
...
def dmain(major_ticker):
    ...
    for ticker in major_ticker:
        ...
        # 각종 이동평균(실행시 한번)
        yes_ma5 = get_yesterday_ma5(-2, df)
        yes2_ma5 = get_yesterday_ma5(-3, df)
        yes3_ma5 = get_yesterday_ma5(-4, df)
        yes_ma20 = get_yesterday_ma20(-2, df)
        yes2_ma20 = get_yesterday_ma20(-3, df)

        # 오늘 시가, 어제 시가, 종가, 거래량, 년 평균 거래량(실행시 한번)
        to_open = get_today_open(df)
        yes_open = get_yesterday_open(df)
        yes_close = get_yesterday_close(df)
        volume_aver = get_aver_volume(df)
        to_volume = get_to_volume(df)
        ...
        jang_dae_minus = (to_open > current_price[ticker]) and (
                abs(to_open - current_price[ticker]) > to_open * jang_minus_cnst)
...
tickers = []
print("BTC", "LINK", "XRP", "ETH", "EOS", "BCH", "TRX", "BSV", "LTC", "ETC", "XLM", "QTUM", "ADA")
s = input("이중 원하는 코인을 공백을 두고 입력하세요 : ").split(" ")

if s == 'all':
    tickers = ["BTC", "LINK", "XRP", "ETH", "EOS", "BCH", "TRX", "BSV", "LTC", "ETC", "XLM", "QTUM", "ADA"]
